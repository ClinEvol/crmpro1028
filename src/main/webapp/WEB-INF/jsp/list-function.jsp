 ...
 <div class="wrapper2 wrapper-content2 animated fadeInRight">
     <div class="ibox float-e-margins">
         <div class="ibox-content">
             <div class="row">
                 <form method="post" action="/programmer/function/list">
                     ...
                     <div class="col-sm-2">
                         <select name="type" class="selectpicker form-control">
                             ...
                             <option value="1" <c:if test="${param.type==1}">selected</c:if> >功能名称</option>
                             <option value="2" <c:if test="${param.type==2}">selected</c:if> >模块名称</option>
                             <option value="3" <c:if test="${param.type==3}">selected</c:if> >需求名称</option>
                             <option value="4" <c:if test="${param.type==4}">selected</c:if> >项目名称</option>
                         </select>
                     </div>
                     ...
                     <div class="col-sm-2 text-right">
                         <a href="/programmer/function/tosave" class="btn btn-sm btn-primary" type="button"><i class="fa fa-plus-circle"></i>添加功能</a>
                     </div>
                 </form>
             </div>
                 ...
                 <table class="table table-striped list-table">
                    ...
                    <c:forEach items="${page.list}" var="function">
                     <tr>
                         ...
                         <td>
                             <a href="/programmer/function/show?id=${function.id}"><i class="glyphicon glyphicon-eye-open  text-navy"></i></a>
                             <a href="/programmer/function/toupdate?id=${function.id}"><i class="glyphicon glyphicon-edit  text-navy"></i></a>
             ...
             <div class="row">
                 ...
                 <div class="col-sm-7 text-right">
                     <div class="btn-group">
                         ...
                         <a href="/programmer/function/list?pageNum=${page.pageNum-1}&type=${param.type}&keyword=${param.keyword}" class="btn btn-sm btn-white  <c:if test="${page.pageNum==1}">disabled</c:if>"><i class="glyphicon glyphicon-chevron-left"></i></a>
                         <c:forEach begin="1" end="${page.pages}" varStatus="index">
                             <a href="/programmer/function/list?pageNum=${index.index}&type=${param.type}&keyword=${param.keyword}" class="btn btn-sm btn-white <c:if test="${index.index==page.pageNum}">active</c:if> ">${index.index}</a>
                         </c:forEach>
                         <a href="/programmer/function/list?pageNum=${page.pageNum+1}&type=${param.type}&keyword=${param.keyword}" class="btn btn-sm btn-white  <c:if test="${page.pageNum==page.pages}">disabled</c:if>"><i class="glyphicon glyphicon-chevron-right"></i></a>
                     </div>
                 </div>
             </div>
 </c:if>
