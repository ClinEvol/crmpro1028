...
<head>
                     ...
                     <form id="saveForm" class="form-horizontal">
                         ...
                         <div class="row">
                             <div class="form-group">
                                 ...
                                 <div class="col-sm-3">
                                     <input  value="${project.customerFk}" type="hidden" id="customerFk" name="customerFk">
                         ...
                         <div class="row">
                             <div class="form-group">
                                 ...
                                 <div class="col-sm-3">
                                     <input  value="${project.employeeCount}" name="employeeCount" type="text" class="form-control input-sm">
                                 </div>
                         ...
                         <div class="row">
                             <div class="form-group">
                                 ...
                                 <div class="col-sm-3">
                                     <input value='<fmt:formatDate value="${project.buildtime}" pattern="yyyy-MM-dd"/>' name="buildtime" id="buid" class="laydate-icon form-control layer-date">
                                 </div>
                             ...
                             <div class="form-group">
                                 ...
                                 <div class="col-sm-3">
                                     <select name="level" class="selectpicker form-control">
                                         ...
                                         <option value="2"<c:if test="${project.level==2}">selected</c:if>>低</option>
                                         <option value="3"<c:if test="${project.level==3}">selected</c:if>>中</option>
                                         <option value="4"<c:if test="${project.level==4}">selected</c:if>>高</option>
                                     </select>
                                 </div>
                                 <label class="col-sm-2 col-sm-offset-1 control-label">计划完成时间</label>
                                 <div class="col-sm-3">
                                     <input value='<fmt:formatDate value="${project.endtime}" pattern="yyyy-MM-dd"/>' name="endtime" id="finish" class="laydate-icon form-control layer-date">
                                 </div>
                             </div>
                         </div>
                         <div class="row">
 ...
 <script>
     $(document).ready(function() {
         ...
         //这里是JS代码，为什么能使用?
         //别忘了这是一个JSP,jsp本质上是java代码，这些代码先编译执行，等到这个解析成html时project.employeeFk}已经是一个具体的值
         //js代码是浏览器解析
         var sysEmployeeFk=${project.employeeFk};
         var sysCustomerFk=${project.customerFk};
         ...
         function loadPM(){
             ...
             $.getJSON("/employee/PMListJSON",function (response) {
                 $(response).each(function () {
                     var option="";
                     if(this.id==sysEmployeeFk){//如果当前对象的id与原本的id相等，说明这个是选 中
         ...
         function loadCustomer(){
             $.getJSON("/programmer/customer/listJSON",function (response) {
                 $(response).each(function () {
                     ...
                     var option="";
                     if(this.id==sysCustomerFk){
                         option="<option selected data-customerid='"+this.id+"' data-person='"+this.companyperson+"' value='"+this.name+"'>"+this.name+"</option>"
                     }else{
                         option="<option data-customerid='"+this.id+"' data-person='"+this.companyperson+"' value='"+this.name+"'>"+this.name+"</option>"
                     }
         ...
         $(".btnSave").click(function () {
             ...
             $.post("/programmer/project/update",data,function (response) {
                 if(response=="true"){
                     //使用甜心弹框提示
                     swal({title:"修改成功",
                             text:"数据已经成功修改，为您返回列表页！",
                             type:"success",
